...
// === PART LOGIN GUEST: LOGIN CANTIK + MODE TAMU PAYWALL ===

wf('components/auth/LoginCard.tsx', `'use client';
import { useState } from 'react';
...
wf('app/login/page.tsx', `import { getClassSettings } from '@/lib/auth/settings-actions';
...
wf('app/guest/page.tsx', `import Link from 'next/link';
...
console.log('[OK] PART LOGIN GUEST done: login cantik + guest paywall');

// === PART GUEST SHELL v2: TAB TERKUNCI + PORTOFOLIO TERBUKA ===

wf('components/guest/GuestShell.tsx', `'use client';
import { useState } from 'react';
import Link from 'next/link';
import { Home, Newspaper, MessageCircle, ClipboardList, Calendar, Image as ImageIcon, Music, Bot, Globe, Lock, LogIn, Brush } from 'lucide-react';
      ...
      <div className="md:pl-64">
        <main className="pb-24 md:pb-8">
          <div className="max-w-5xl mx-auto px-4 py-8 space-y-8">
            <div className="text-center space-y-2">
              <h1 className="text-2xl md:text-4xl font-black">
                Intip <span className="text-grad">{settings?.class_name || 'ClassHub'}</span>
              </h1>
              <p className="text-sm text-mut">
                Kamu masuk sebagai tamu. Semua konten terkunci kecuali Portofolio — login buat ikut di dalamnya.
              </p>
            </div>
            ...
            <div className="relative">
              ...
              <div className="absolute inset-0 flex items-center justify-center p-4">
                <div className="bg-card/90 backdrop-blur-xl border border-line rounded-3xl p-6 md:p-8 text-center space-y-3 max-w-sm shadow-2xl">
                  ...
                  <h2 className="text-lg font-bold text-ink">Anda harus login terlebih dahulu</h2>
                  <p className="text-sm text-mut">
                    Konten kelas ini privat buat anggota {settings?.class_name || 'kelas'}. Minta akun ke admin kelas, lalu masuk buat lihat feed, tugas, galeri, dan lainnya.
                  </p>
                  <Link href="/login" className="inline-flex w-full items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-acc text-acc-ink text-sm font-semibold">
                    <LogIn className="h-4 w-4" />
                    Login Sekarang
                  </Link>
                </div>
...
wf('app/guest/page.tsx', `import { getClassSettings } from '@/lib/auth/settings-actions';
...
console.log('[OK] GUEST SHELL v2 done: tab terkunci + portofolio terbuka');

// === PART GUEST SHELL v3: WALL CUMA SAAT DIPENCET ===
...
console.log('[OK] GUEST SHELL v3 done: wall cuma saat dipencet');