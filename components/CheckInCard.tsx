...
export default function CheckInCard() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      if (s && s.error) { setErrMsg(s.error); setPhase('error'); return; }
      setStatus(s);
      setPhase('ready');
    } catch (e: any) {
  ...
  async function doCheckIn() {
    ...
    setBusy(false);
    if (res && res.error) { setErrMsg(res.error); return; }
    if (typeof window !== 'undefined') window.dispatchEvent(new Event('checkin-done'));
    await load();
  }
